          ...
          this.faceMeshDetector
            ...
            .pipe(
              ...
              tap(predictions => {
                ...
                if (((predictions as AnnotatedPrediction[])?.length === this.totalFaces) && predictions[0].faceInViewConfidence >= this.confidenceScore) {
                  this._detectFacesResult = { size: (predictions as AnnotatedPrediction[])?.length, encodedURI: Canvas.readAsDataURL(canvas) };
                }
                ...
                this.faceMeshDrawer.drawFacePoints(
                  context
                )((predictions as AnnotatedPrediction[]) || []);
              })
            ).subscribe();
        }
      },
